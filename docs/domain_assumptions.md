...
| **`select_by="de"` decouples membership from the proxy** | DE gates define the list; nascent gates are skipped; sorting is by `p_adj` then `logFC`. Primary path uses this mode. | pass `select_by="composite"` for legacy proxy-aware filtering — not recommended for production lists |
...
| **Genome-wide zero ≠ gene-set zero** | `transcription_support` is standardized over the run’s gene universe; a curated program can still have a non-zero expectation under no condition effect (structural offset from length/intron/expression — e.g. ARE on GSE226488 S5). | Absolute placement: `program_mechanism_permutation_calibrated` (observed − label-shuffle null); do not read raw program means as absolute |
| **Program membership must be frozen under the null** | `program_mechanism_permutation_calibrated` requires a precomputed `de=` table so DE re-selection does not enter the null. | Recompute DE only for the observed pass; reuse that table for every replicate |
| **`null_sd` is offset stability** | SD of label-shuffle program means; not the sampling SE of the observed mean. | Interpreting `null_sd` as SE($m$) in a classical one-sample $z$-test without stating that choice |
