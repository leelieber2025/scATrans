...
These are the current keyword defaults. Recommendations elsewhere in the
docs do not change them.
...
`run_default_pipeline` records `meta["regime"]` from
`scat.qc.regime_diagnosis` (global unspliced fraction). If the diagnostic
raises, the result is still returned. Optional add-ons skip missing
columns and record the error in `meta`; an invalid `bias_method` raises.
