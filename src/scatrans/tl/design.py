...
def assign_pseudo_replicates(
    ...
    conditions = list(pd.unique(adata.obs[groupby].dropna()))
    ...
    for cond in conditions:
        ...
        if n_cells < n_replicates:
            raise ValueError(
                f"assign_pseudo_replicates: group {cond!r} has {n_cells} cell(s) "
                f"but n_replicates={n_replicates}; need at least n_replicates "
                "cells per group level."
            )
        ind_labels = [f"{cond}_rep{i + 1}" for i in range(n_replicates)]
        tiled = np.tile(
            np.asarray(ind_labels, dtype=object),
            int(np.ceil(n_cells / n_replicates)),
        )[:n_cells]
        labels.iloc[np.flatnonzero(mask)] = rng.permutation(tiled)
