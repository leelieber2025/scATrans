...
**ggVolcano-style volcano plots** ([BioSenior/ggVolcano](https://github.com/BioSenior/ggVolcano)) are available via `style=`:
...
- `scat.pl.volcano_plot(results_df, top_n=10, label_genes=None, style="auto", ...)`  
  ...
  - **`style="ggvolcano"`**: [ggVolcano](https://github.com/BioSenior/ggVolcano) classic — teal Down / grey Normal / orange Up, `theme_bw` grid, dashed cutoffs, FDR-ranked labels, in-axes legend (`legend_position="UL"`).
  - **`style="gradual"`**: ggVolcano `gradual_volcano` — gradient colour and point size by `-log10(FDR)`.
...
**Software (Python source):** [Apache License 2.0](LICENSE).
...
| GO Biological Process `*_2026.txt` | GO / Bioconductor-derived — see [DATA_LICENSES.md](src/scatrans/data/DATA_LICENSES.md) |
...
If you ship a product or offer a commercial service, review [DATA_LICENSES.md](src/scatrans/data/DATA_LICENSES.md) before using bundled KEGG mappings. You can avoid bundled KEGG files by passing an Enrichr/gseapy library name (e.g. `kegg_library="KEGG_2021"`) or your own `gene_sets` file.
