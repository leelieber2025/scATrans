...
`active_score` (0–100) is a **heuristic ranking score** for visualization and sorting. It is not a probability.
...
The permutation shuffles only group labels; unspliced/spliced layers and the reference gamma are fixed from the original labeling for speed. This approximation is documented in `permutation_approximation_note`.
